...
@router.post("/auth")
async def authenticate_user(payload: TokenAuth, request: Request, db: AsyncSession = Depends(get_db)):
    try:
    ...
    if user_id := next((uid for uid in session_store if session_store[uid]["timestamp"]), None):

...
async def api_get_user_portfolio(
    ...
    portfolio = portfolio_to_text(await fetch_portfolio_from_service(
        portfolio_id=payload.portfolio_id,
        user_id=payload.user_id,
    ))
    if not portfolio:
...
async def api_get_user_profile(
    ...
    profiles = await fetch_profile_from_service(payload.portfolio_id, payload.user_id)
    ...
    if question:
        ...
        updatable_fields = [
            f for f in UserProfileBase.model_fields.keys() if f not in exclude_fields
        ]
        ...
        if specific:
            ...
            for field in updatable_fields:
                ...
                if value not in (None, ""):
                    ...
                    if (
                        ...
                        and field.endswith("objectives")
                        or field.endswith("preferences")
                    ):
            ...
            for field in updatable_fields:
                current_profile[field] = (
                    []
                    if (field.endswith("objectives") or field.endswith("preferences"))
                    else None
                )

        update_fields = await extract_profile_details(question, current_profile)
        if update_fields:
            updated_profile = await update_user_profile_fields(

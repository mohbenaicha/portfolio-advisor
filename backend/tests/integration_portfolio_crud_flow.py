...
async def test_portfolio_crud_flow():
    async with httpx.AsyncClient() as client:
        ...
        assert res.status_code in [403, 404], "❌ Unauthorized access not blocked"
        ...
        assert res.status_code in [200, 201], f"❌ Portfolio creation failed: {res.text}"
        portfolio = res.json()
        ...
        res = await client.get(f"{BASE_URL}/portfolios/{portfolio_id}", headers=headers)
        assert res.status_code == 404, "❌ Deleted portfolio still accessible"
